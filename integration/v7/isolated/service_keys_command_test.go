...
var _ = Describe("service-keys command", func() {
	...
	When("there is a service instance", func() {
		...
		BeforeEach(func() {
			...
			helpers.SetupCF(orgName, spaceName1)
		...
		When("there are service keys", func() {
			...
			BeforeEach(func() {
				...
				Eventually(helpers.CF("share-service", serviceInstanceName, "-s", spaceName2)).Should(Exit(0))

				helpers.TargetOrgAndSpace(orgName, spaceName2)
